...
IF snname[i] EQ 'sn1993J' then cd,basicdir+snname[i]+'/spec/lick' ELSE if snname[i] EQ 'sn2007ru' then cd,basicdir+snname[i]+'_Sahu' ELSE if snname[i] EQ 'sn2010bh' then cd, basicdir+snname[i]+'/spec/Chornock' ELSE cd,basicdir+snname[i]+'/spec'
   ...
   if Si_flag eq 0 then begin
    openw,2,'/Users/yuqianliu/Desktop/snidmeanspectra/templates-2.0_myself/forConvolution/'+snname[i]+'_FeII5169_sigma_conv.dat', width=120
   ...
   if Si_flag eq 1 then begin
    openw,2,'/Users/yuqianliu/Desktop/snidmeanspectra/templates-2.0_myself/forConvolution/'+snname[i]+'_SiII_sigma_conv.dat', width=120
    if snname[i] eq 'sn2010bh' or snname[i] eq 'sn2010ma' or snname[i] eq 'PTF10vgv' then begin
   ...
   printf, 2, 'spec_name  ', 'phase        ','sigma_50         ','sigma_err_16  ','sigma_err_84   '  
   phases=round(phase)
   nspec=n_elements(phases)
    ...
    dum8 = strsplit(dum8,/extract)   
    dum8_0=strsplit(dum8[0], /extract, '[')
    vel_conv_1=-gettok(dum8_0, ',')*4000.0 ; 16 percentile
    vel_conv_2=-gettok(dum8[2], ',')*4000.0 ; 84 percentile
    vel_conv_12=-gettok(dum8[1], ',')*4000.0 ; 50 percentile
    ...
    dum8 = strsplit(dum8,/extract)   
    dum8_0=strsplit(dum8[0], /extract, '[')
    vel_conv_1=-gettok(dum8_0, ',')*4000.0
    vel_conv_12=-gettok(dum8[1], ',')*4000.0
    vel_conv_2=-gettok(dum8[2], ']')*4000.0
    vel_total=vel_conv_12;+vel_mean
    vel_err1=float(vel_conv_1-vel_conv_12)
    vel_err2=float(vel_conv_12-vel_conv_2)
    ;vel_err1_temperr=sqrt(vel_err1^2+err_std(where(phase_std eq phase_use))^2)
